...
**ShopGraph** makes your WooCommerce catalog first-class for AI shopping agents:
...
* **No duplicate schema.** ShopGraph detects Yoast SEO and Rank Math and merges its data into their existing Product node instead of printing a second one. With no SEO plugin, it prints a standalone node.
...
* **Feature toggles** under WooCommerce → Settings → ShopGraph.

ShopGraph reads product data exclusively through the WooCommerce CRUD API and declares High-Performance Order Storage (HPOS) compatibility.

== Installation ==

1. Upload the plugin to `/wp-content/plugins/shopgraph`, or install it from the Plugins screen.
2. Activate it. WooCommerce must be installed and active.
3. Go to WooCommerce → Settings → ShopGraph to toggle features.
...
No. ShopGraph detects those plugins and merges its AI attributes into their existing Product schema node. It only prints a standalone node when no supported SEO plugin is active (or when you force "Standalone" mode).
